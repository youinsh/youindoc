...
## 频道装修-播放水印信息获取
```
GET /api/v2/decoration/watermark/info/{{id}}/
```
### 参数
...
```json
{
    "result": {
        ...
        "course": "10",
        "enterprise": "2",
        "watermark_show": false,//是否显示水印效果
        "watermark_position": "topleft",//位置  topleft：左上，topright：右上，lowerleft：左下，lowerright：右下
        "water_url": "static/jkwhfwe.jpg",//水印图片地址
        "create_time": "2019-08-21T09:58:19.817084"
    },
    "msg": "success",
    "code": 200
}
...
## 频道装修-播放器水印 新建或者修改
```
POST /api/v2/decoration/watermark/edit/
```
### 参数
...
|----|----|----|----|
|course|`str`|是|频道id|
|watermark_show|`str`|否|是否显示 0:不显示，1显示|
|watermark_position|`str`|是|显示位置 topleft：左上，topright：右上，lowerleft：左下，lowerright：右下|
|water_url|`str`|否|图片oss路径|
```
如若为新建，则所有参数都必须有，
如为修改，则带入修改都字段
```
...
## 频道装修-微信公众号二维码信息
```
GET /api/v2/decoration/wechatqrcode/info/{{id}}/
```
### 参数
...
```json
{
    "result": {
        ...
        "course": "10",
        "enterprise": "2",
        "qrcode_show": true,    //是否显示二维码
        "pubilc_name": "我的公众号",//显示的公众号名称
        "qrcode_url": "jhgfw/jhewf.jpg",//二维码图片
        "enter_show": false,//是否进入播放页面弹出弹框显示
        "create_time": "2019-08-21T10:00:19.834352"
    },
    "msg": "success",
    "code": 200
}
...
## 频道装修-微信公众号二维码 新建或者修改
```
POST /api/v2/decoration/wechatqrcode/edit/
```
### 参数
...
|----|----|----|----|
|course|`str`|是|频道id|
|qrcode_show|`str`|否|是否显示 0:不显示，1显示|
|pubilc_name|`str`|是|显示的公众号名称|
|qrcode_url|`str`|否|图片oss路径|
|enter_show|`str`|否|是否弹框显示 0:不弹框，1:弹框|
```
如若为新建，则所有参数都必须有，
如为修改，则带入修改都字段
```
...
Content-Type: application/json
{
    "course": "10",
    "qrcode_show": 1,
    "pubilc_name": "我的公众号111",
    "qrcode_url": "jhgfw/qwwqwqwq.jpg",
    "enter_show": 0
}
```
### 返回
...
## 频道装修-自定义菜单栏获取信息
```
GET /api/v2/decoration/custommenu/info/{{id}}/
```
### 参数
...
```json
{
    ...
    "result": [
        {
            "id": 3,
            "menu_name": "聊天互动",//菜单显示名称
            "menu_type": "chat",//菜单类型（暂时只支持以下 text：图文，video：视频，chat：聊天互动，rank：榜单排行）
            "content_type": null,//当为video则会关联视频表，这个字段会有数据
            "association": null,//当关联了视频，这个代表视频表的id
            "content": ""//图文模式显示的内容
        },
        {
            "id": 4,
...
## 频道装修-自定义菜单新建
```
POST /api/v2/decoration/custommenu/add/
```
### 参数
...
|----|----|----|----|
|course|`str`|是|频道id|
|menu_name|`str`|是|是否显示 0:不显示，1显示|
|menu_type|`str`|是|显示的公众号名称|
|content|`str`|是|图片oss路径|
|association|`str`|否|关联当视频id（此字段目前不需要，暂未开发）|
```
如若为新建，则所有参数都必须有，
如为修改，则带入修改都字段
```
...
Content-Type: application/json
{
    ...
    "menu_type": "text",
    "content":"今晚和放开我。。",
    "association":"1"//当type为video关联视频，此字段必须，其他不需要带入
}
```
### 返回
```json
...
## 频道装修-自定义菜单 修改
```
POST /api/v2/decoration/custommenu/edit/{{id}}
```
### 参数

|名称|类型|是否必须|描述|
|----|----|----|----|
|id|`str`|是|获取到到自定义菜单的id|
|menu_name|`str`|是|是否显示 0:不显示，1显示|
|menu_type|`str`|是|显示的公众号名称|
|content|`str`|是|图片oss路径|
...
```json
{
    ...
    "result": {
        ...
        "association": null,
        "content": "qwqwqw",
        "create_time": "2019-08-21T17:51:35.281687"
    }
}
```
...
## 频道装修-自定义菜单 删除
```
POST /api/v2/decoration/custommenu/del/{{id}}
```
### 参数
